...
==============================================================  
Контрольные вопросы:
- Node – это отдельная машина (физическая или виртуальная) в кластере, на которой могут запускаться поды.
- Cluster – совокупность всех узлов и управляющих компонентов, которые работают как единая система.  

- В моём кластере control plane находится на ноде controlplane, а worker — на ноде node01. Это видно в выводе kubectl get nodes по колонке ROLES. Все прозрачно.
